...
import sys
import cgi          # <-- import cgi for FieldStorage
import cgitb       # <-- import cgitb for debugging
import pymysql
import http.cookies
import os
...
search_query = form.getvalue("q", "").strip()
# Ensure UTF-8 output
...
<nav class="navbar navbar-expand-lg navbar-dark bg-danger sticky-top">
  <div class="container">
    ...
    <form class="d-flex ms-auto me-3" method="get" action="home.py">
        <input class="form-control me-2" type="search" placeholder="Search for dishes..." name="q" value="{search_query}">
        <button class="btn btn-light" type="submit"><i class="fas fa-search"></i></button>
      </form>
      <ul class="navbar-nav ms-auto">
    <li class="nav-item">
        <a class="nav-link" href="home.py"><i class="fas fa-home"></i> Home</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="cart.py"><i class="fas fa-shopping-cart"></i> Cart</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="profile_user.py"><i class="fas fa-user-circle"></i> Profile</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="orders_user.py"><i class="fas fa-clipboard-list"></i> My Orders</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="logout.py"><i class="fas fa-sign-out-alt"></i> Logout</a>
    </li>
</ul>
...
for pid, name, desc, price, img in products:
    # Correct image path
    img_path = img if img and img.startswith("uploads/") else f"uploads/{img}"

    # Define view button
    view_button = f'<a class="btn btn-secondary me-2" href="view_product.py?id={pid}">View</a>'
